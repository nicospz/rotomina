...
class EevxAdapter:
    ...
    async def devices(self):
        ...
        # Only discovery fields; never forward credentials from /mapping.
        return [{'id': device_uuid(d.get('id')), 'name': d.get('name', '')}
                for d in data['devices'] if isinstance(d, dict) and not d.get('revoked')]
